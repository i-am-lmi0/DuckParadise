...
async def mute(ctx, member: discord.Member, duration: str = None, *, reason=None):
    ...
    try:
        await member.send(f"You have been muted in {ctx.guild.name} for: {reason} ({duration or 'indefinitely'})")
    except:
        await log_action(ctx, f"Failed to DM mute message to {member} (ID: {member.id})")
    if duration:
        ...
        if seconds:
            ...
            try:
                await member.send(f"You have been automatically unmuted in {ctx.guild.name} after {duration}.")
            except:
                await log_action(ctx, f"Failed to DM unmute message to {member} (ID: {member.id})")
        else:
