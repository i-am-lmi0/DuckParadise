...
async def log_action(ctx, message):
    if log_channel_id:
...
async def kick(ctx, member: discord.Member, *, reason=None):
    ...
    await log_action(ctx, f"kicked {member} for: {reason}")
...
async def ban(ctx, member: discord.Member, *, reason=None):
    ...
    await log_action(ctx, f"banned {member} for: {reason}")
...
async def unban(ctx, user_id: int):
    ...
    await log_action(ctx, f"unbanned {user}")
...
async def mute(ctx, member: discord.Member, duration: str = None, *, reason=None):
    ...
    if duration:
        ...
        if seconds:
            ...
            await log_action(ctx, f"{member} was automatically unmuted after {duration}")
        else:
...
async def unmute(ctx, member: discord.Member):
    ...
    if mute_role in member.roles:
        ...
        await log_action(ctx, f"unmuted {member}")
    else:
...
async def warn(ctx, member: discord.Member, *, reason=None):
    ...
    await log_action(ctx, f"warned {member} for: {reason}")
